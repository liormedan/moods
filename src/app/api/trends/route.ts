...
function generateTrendData(range: string, type: string) {
  ...
  const baseData = periods.map((period, index) => {
    ...
    return {
      period,
      moodAverage: Math.max(1, Math.min(10, 6.5 + seasonalFactor + trendFactor + (Math.random() - 0.5) * 2)),
      journalEntries: Math.floor(8 + seasonalFactor * 3 + Math.random() * 10),
      breathingMinutes: Math.floor(15 + seasonalFactor * 5 + Math.random() * 20),
      goalsCompleted: Math.floor(2 + trendFactor + Math.random() * 3),
      activeDays: Math.floor(20 + seasonalFactor * 5 + Math.random() * 8),
      sleepQuality: Math.max(1, Math.min(10, 7 + seasonalFactor * 0.5 + (Math.random() - 0.5) * 2)),
      stressLevel: Math.max(1, Math.min(10, 5 - seasonalFactor * 0.3 + (Math.random() - 0.5) * 2)),
      energyLevel: Math.max(1, Math.min(10, 6.5 + seasonalFactor + (Math.random() - 0.5) * 1.5)),
      socialActivity: Math.max(1, Math.min(10, 6 + seasonalFactor * 0.8 + (Math.random() - 0.5) * 2)),
    };
  });
  ...
  switch (range) {
    case '6months':
      for (let i = 5; i >= 0; i--) {
        ...
        periods.push(date.toLocaleDateString('he-IL', { year: 'numeric', month: 'long' }));
      }
      break;
    case 'year':
      for (let i = 11; i >= 0; i--) {
    ...
    case 'all':
      ...
      break;
    default:
      periods.push(now.toLocaleDateString('he-IL', { year: 'numeric', month: 'long' }));
  }
...
function generatePredictions(trendData: any[]) {
  ...
  const avgTrend = lastFew.reduce((sum, item, index) => {
    if (index === 0) return 0;
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    const predicted = Math.max(1, Math.min(10, lastMood + avgTrend * i + (Math.random() - 0.5) * 0.5));
    
    predictions.push({
      period: futureDate.toLocaleDateString('he-IL', { year: 'numeric', month: 'long' }),
...
function generateInsights(trendData: any[], seasonalData: any[], correlations: any[]) {
  const insights = [];
  
  // Trend analysis
  const recentTrend = trendData.slice(-3).reduce((sum, item) => sum + item.moodAverage, 0) / 3;
  const earlierTrend = trendData.slice(0, 3).reduce((sum, item) => sum + item.moodAverage, 0) / 3;
  
  if (recentTrend > earlierTrend + 0.5) {
    insights.push('מצב הרוח שלך משתפר בהדרגה! המגמה החיובית מעידה על התקדמות טובה.');
  } else if (recentTrend < earlierTrend - 0.5) {
    insights.push('נצפתה ירידה במצב הרוח לאחרונה. כדאי לשקול הגברת פעילויות מועילות.');
  } else {
  ...
  insights.push(`${bestSeason.season} היא העונה הטובה ביותר עבורך. כדאי לתכנן פעילויות מיוחדות בתקופה זו.`);

  // Activity insights
  const avgBreathing = trendData.reduce((sum, item) => sum + item.breathingMinutes, 0) / trendData.length;
  if (avgBreathing > 20) {
    insights.push('אתה מתרגל נשימה באופן קבוע - זה תורם משמעותיות לרווחה שלך!');
  } else {
    insights.push('הגברת תרגילי הנשימה יכולה לשפר את מצב הרוח שלך בצורה משמעותית.');
  }

  // Goals insights
  const avgGoals = trendData.reduce((sum, item) => sum + item.goalsCompleted, 0) / trendData.length;
  if (avgGoals > 2.5) {
    insights.push('אתה מצליח להשלים מטרות באופן עקבי - המשך כך!');
  } else {
    insights.push('הגדרת מטרות קטנות יותר ויותר ברות השגה יכולה לשפר את תחושת ההצלחה.');
  }

  // Correlation insights
  const strongestPositive = correlations.filter(c => c.correlation > 0).reduce((max, current) => 
    current.correlation > max.correlation ? current : max
  );
  insights.push(`${strongestPositive.factor} הוא הגורם החיובי החזק ביותר עבורך - כדאי להתמקד בו.`);
